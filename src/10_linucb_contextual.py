...
def load_returns(path: Path) -> pd.DataFrame:
    ...
    df = df.sort_index()
    # Normaliza a fecha (sin hora) para empatar con stems YYYY-MM-DD.npy
...
def ucb_scores(st: LinUCBState, X: np.ndarray, alpha: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    mu = X @ th         # (K,)
    XA = X @ st.A_inv   # (K,d)
    sigma = np.sqrt(np.sum(XA * X, axis=1))  # sqrt(x^T A_inv x) por brazo
    ucb = mu + alpha * sigma
    return ucb, mu, sigma
...
def run_policy(
    ...
    # Validación: returns debe contener todas las fechas de contextos
    ...
    # Determinar K y d desde el primer snapshot
    ...
    # Loop causal: decide con X_t, observa reward en t+1
    for t in range(T - 1):
        date_t = dates[t]
        date_next = dates[t + 1]
        ...
        # Regret empírico: vs mejor activo en hindsight en esa semana
...
def main():
    ...
    summaries = []
    summaries.append(run_policy("linucb", context_dir, returns, dates, args.alpha, args.lam, reward_clip, args.seed, out_dir))
    summaries.append(run_policy("greedy", context_dir, returns, dates, 0.0, args.lam, reward_clip, args.seed, out_dir))
    summaries.append(run_policy("random", context_dir, returns, dates, 0.0, args.lam, reward_clip, args.seed, out_dir))
