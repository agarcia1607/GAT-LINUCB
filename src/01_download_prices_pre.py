...
def safe_to_parquet(df: pd.DataFrame, parquet_path: str, csv_path: str) -> str:
    """Try Parquet, fallback to CSV if parquet engine missing."""
    try:
...
def update_metadata(download_done: bool, saved_path: str, n_tickers: int, n_dates: int):
...
def main():
    ...
    tickers = load_tickers(ART_TICKERS)
